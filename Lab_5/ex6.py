# Exercise 6. Effect of the process noise covariance Q
...
# ---- Kalman filter as a function of Q ----
def run_kalman(Q_filter, y, x0=0.0, P0=1.0):
    x_est = np.zeros(N+1)
    P_est = np.zeros(N+1)
    K_all = np.zeros(N+1)
...
# ---- Run for three Q values ----
Q_values = [0.0001, 0.01, 1.0]
colors   = ['tab:blue', 'tab:green', 'tab:red']
results  = {}

for Q in Q_values:
    x_est, P_est, K_all = run_kalman(Q, y_meas)
    mse = np.mean((x_true[1:] - x_est[1:])**2)
    results[Q] = {'x_est': x_est, 'P': P_est, 'K': K_all, 'MSE': mse}

# ---- Print the MSE table ----
print("===== Effect of Q on Kalman filter =====")
print(f"{'Q':>10} | {'MSE':>8} | {'K_ss':>8} | {'P_ss':>8}")
print("-" * 42)
for Q in Q_values:
    r = results[Q]
    print(f"{Q:>10.4f} | {r['MSE']:>8.4f} | {r['K'][-1]:>8.4f} | {r['P'][-1]:>8.4f}")

time = np.arange(N+1)

# ======================================================================
# Plot 1: estimates for all three Q values, vs truth and measurements
# ======================================================================
...
for Q, col in zip(Q_values, colors):
    ...
    plt.plot(time, r['x_est'], '-', color=col, linewidth=2,
             label=f'Kalman, Q={Q}  (MSE={r["MSE"]:.3f})')

plt.xlabel('Time step t')
plt.ylabel('Value')
plt.title('Kalman Estimate for Different Values of Q')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
# ======================================================================
# Plot 2: Kalman gain and covariance for all three Q
# ======================================================================
...
ax1.set_title('Kalman Gain and Posterior Covariance for Different Q')
ax1.legend()
ax1.grid(True)
